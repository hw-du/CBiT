...
class CombinatorialEnumerate(object):
    ...
    """
    def __init__(self, truncation_rate=0.2, gamma=0.7, beta=0.2, \
                item_similarity_model=None, insert_rate=0.3, \
                max_insert_num_per_pos=3, substitute_rate=0.3, n_views=5):
        self.data_augmentation_methods = [Truncation(truncation_rate=truncation_rate), Mask(gamma=gamma), Reorder(beta=gamma),
...
class Random(object):
    ...
    def __init__(self, truncation_rate=0.2, gamma=0.7, beta=0.2, \
        ...
        if self.augment_threshold == -1:
            self.data_augmentation_methods = [Truncation(truncation_rate=truncation_rate), Mask(gamma=gamma), Reorder(beta=beta),
                                Insert(item_similarity_model, insert_rate=insert_rate, 
                                    max_insert_num_per_pos=max_insert_num_per_pos),
                                Substitute(item_similarity_model, substitute_rate=substitute_rate)]
            ...
            if self.augment_type_for_short == 'SI':
                ...
                self.short_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate, 
                                        ...
                                        augment_threshold=self.augment_threshold),
                                    Substitute(item_similarity_model, substitute_rate=substitute_rate),
                                    Truncation(truncation_rate=truncation_rate)]
                ...
                self.short_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate, 
                                        ...
                                        augment_threshold=self.augment_threshold),
                                    Substitute(item_similarity_model, substitute_rate=substitute_rate),
                                    Mask(gamma=gamma), Truncation(truncation_rate=truncation_rate)]
            elif self.augment_type_for_short == 'SIRC':
                self.short_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate, 
                                        max_insert_num_per_pos=max_insert_num_per_pos, 
                                        augment_threshold=self.augment_threshold),
                                    Substitute(item_similarity_model, substitute_rate=substitute_rate),
                                    Reorder(beta=gamma), Truncation(truncation_rate=truncation_rate)]
            else:
                ...
                self.short_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate, 
                                        ...
                                        augment_threshold=self.augment_threshold),
                                    Substitute(item_similarity_model, substitute_rate=substitute_rate),
                                   Truncation(truncation_rate=truncation_rate), Mask(gamma=gamma), Reorder(beta=gamma)]
...
class Truncation(object):
    """Randomly Truncation a subseq from the original sequence"""
    def __init__(self, truncation_rate=0.2):
        self.truncation_rate = truncation_rate

    def __call__(self, sequence):
        # make a deep copy to avoid original sequence be modified
        copied_sequence = copy.deepcopy(sequence)
        sub_seq_length = int(self.truncation_rate*len(copied_sequence))
        #randint generate int x in range: a <= x <= b
        start_index = random.randint(0, len(copied_sequence)-sub_seq_length-1)
        if sub_seq_length<1:
            return [copied_sequence[start_index]]
        else:
            Truncation_seq = copied_sequence[start_index:start_index+sub_seq_length]
            return Truncation_seq

class Mask(object):
    """Randomly mask k items given a sequence"""
    def __init__(self, gamma=0.7, mask_token=0):
        self.gamma = gamma
        self.mask_value=mask_token
    ...
    def __call__(self, sequence):
        ...
        for idx in mask_idx:
            copied_sequence[idx] = self.mask_value
        return copied_sequence
